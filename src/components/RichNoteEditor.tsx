...
export const RichNoteEditor: React.FC<RichNoteEditorProps> = ({
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${barClasses}`} style={{ ['--rich-note-accent' as any]: accentColor }}>
      <div
      ...
      <EditorContent
        editor={editor}
        className={`px-3 py-2.5 max-h-80 overflow-y-auto ${isDark ? 'text-slate-100' : 'text-slate-900'}`}
      />
    </div>
  );
};
